...
export function EmployeeForm({ initialData, readOnly = false }: EmployeeFormProps) {
  ...
  async function onSubmit(data: EmployeeFormValues) {
    if (readOnly) return; // Block submit in readOnly mode
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="max-w-5xl mx-auto">
        ...
        {/* NOTE: Fieldset removed from here so Tabs remain clickable */}
        ...
        {/* 3. BUTTON TOGGLE: Only show if NOT readOnly */}
        {!readOnly && (
